...
|---|---|---|---|
| MySQL 8.0 | `mysql:8.0` | 3306 | `root` / `Lpr#Dev_2026!` |
...
|---|---|---|
| `Camaras` | `Id` (PK identity), `Codigo` (único), `Nombre`, `Latitude`/`Longitude` (`decimal(9,6)`), `TipoInstalacion` (string: `ArcoSeguridad`\|`AvenidaAltaVelocidad`), `VelocidadMaximaKmh`, `Activa`, `CreatedAtUtc` | `Latitude`/`Longitude` usan WGS84 — mismo sistema de coordenadas que ESRI/Google Maps |
...
- [x] Entidades + `LprDbContext` + migración inicial con índice compuesto en `LecturasHistoricas` (`Core.Domain`, Fase 1)
